...
export FLASK_APP=anonimus.tdk.rise
...
format:
	isort ${ISORT_FLAGS} --check-only --diff ${NAME} anonimus test
	black ${BLACK_FLAGS} --check --diff ${NAME} anonimus test
	autoflake ${FLAKE_FLAGS} --in-place ${NAME} anonimus test

format-fix:
	isort ${ISORT_FLAGS} ${NAME} anonimus test
	black ${BLACK_FLAGS} ${NAME} anonimus test
	autoflake ${FLAKE_FLAGS} ${NAME} anonimus test
